...
public class SeamCarver {
    ...
    public double energy(int x, int y) {
        ...
        double dx21 = Math.pow(picture.get((x - 1 + width) % width, y).getRed()
                - picture.get((x + 1) % width, y).getRed(), 2);
        double dx22 = Math.pow(picture.get((x - 1 + width) % width, y).getGreen()
                - picture.get((x + 1) % width, y).getGreen(), 2);
        double dx23 = Math.pow(picture.get((x - 1 + width) % width, y).getBlue()
                - picture.get((x + 1) % width, y).getBlue(), 2);
        double dx2 = dx21 + dx22 + dx23;
        double dy21 = Math.pow(picture.get(x, (y - 1 + height) % height).getRed()
                - picture.get(x, (y + 1) % height).getRed(), 2);
        double dy22 = Math.pow(picture.get(x, (y - 1 + height) % height).getGreen()
                - picture.get(x, (y + 1) % height).getGreen(), 2);
        double dy23 = Math.pow(picture.get(x, (y - 1 + height) % height).getBlue()
                - picture.get(x, (y + 1) % height).getBlue(), 2);

...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void sink(int index) {
        ...
        int j = min(leftIndex(index), rightIndex(index));
        while (inBounds(j) && contents[index].priority() > contents[j].priority()) {
            swap(index, j);
            index = j;
            j = min(leftIndex(j), rightIndex(j));;
        }
    }
    ...
    public void changePriority(T item, double priority) {
        ...
        contents[num].myPriority = priority;
        if (priority < contents[parentIndex(num)].priority())
            swim(num);
        if (priority > contents[min(leftIndex(num), rightIndex(num))].priority())
            sink(num);
    }
